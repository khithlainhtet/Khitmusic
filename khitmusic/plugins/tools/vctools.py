...
from khitmusic import app, Userbot
...
from khitmusic.utils.admin_filters import admin_filter
from config import BANNED_USERS

@app.on_message(filters.command(["vcinfo"], ["/", "!"]))
async def strcall(client, message):
    assistant = await group_assistant(SANYA, message.chat.id)
    try:
        await assistant.join_group_call(message.chat.id, AudioPiped("./khitmusic/assets/call.mp3"), stream_type=StreamType().pulse_stream)
...
async def set_volume(client, message: Message):
    ...
    try:
        await SANYA.change_volume(chat_id, volume_level)
